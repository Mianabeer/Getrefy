...
export const Sidebar: React.FC = () => {
  const { activeView, setActiveView, userProfile, showToast, unreadNotificationCount, openOnboarding } = useApp();
  ...
  const navItems: { id: ActiveView; label: string; icon: any; badge?: string }[] = [
    ...
    { id: 'submit', label: 'Create Post', icon: PlusSquare, badge: '+3pts' },
      ...
      <div className="space-y-3 relative">
            ...
            {isUserMenuOpen && (
              <div className="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-[#0E0E10] border border-[#E5E5E5] dark:border-[#2A2A2C] rounded-2xl shadow-2xl p-1.5 z-40 animate-in fade-in slide-in-from-bottom-2 duration-150">
                ...
                <button
                  ...
                  className="w-full px-3 py-2 text-left text-xs text-[#2563EB] hover:bg-[#2563EB]/10 rounded-xl flex items-center gap-2 font-bold transition-colors cursor-pointer"
                >
                  <HelpCircle className="w-3.5 h-3.5 text-[#2563EB]" />
                  <span>How Getrefy Works</span>
                </button>
        ...
        <div className="px-2 space-y-1 text-center">
          <button
            ...
            className="text-[11px] font-bold text-[#2563EB] hover:underline inline-flex items-center gap-1 cursor-pointer"
          >
            <HelpCircle className="w-3 h-3" />
            <span>How Getrefy Works</span>
          </button>
          <div className="text-[10px] text-[#1A1A1B]/40 dark:text-[#F5F5F5]/40 font-medium">
